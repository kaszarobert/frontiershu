...
# Copy everything from source to destination, but convert "xml" files
find "$source_folder" -type f -exec sh -c '
    ...
    # Check if the source file has a "xml" extension
    if [ -n "$(echo "$source_file" | grep -o '\.xml\$')" ]; then
        # Convert the "xml" file to "cnvrs-text" format using PuyoTextEditor
        destination_file="${destination_file%.xml}.cnvrs-text"

        # Create the destination directory if it does not exist
        destination_dir=$(dirname "$destination_file")
        mkdir -p "$destination_dir"

        PuyoTextEditor "$source_file" -o "$destination_file"  -f cnvrs-text
